...
void *threads(void *arg)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        count = count + 1;
        if (count == n)
            sem_post_count(turnstile, n);   // unlock the first
        sem_post(mutex);
...
int main(void)
{
    ...
    // Create the semaphore
    mutex = sem_open_temp("sem_prog3.7.6_m", 1);
    turnstile = sem_open_temp("sem_prog3.7.6_t", 0);
    turnstile2 = sem_open_temp("sem_prog3.7.6_t2", 0);
